...
{
    int index = threadIdx.x * blockDim.x + threadIdx.x;  // Get the thread's unique index within the block.
    int stride = blockDim.x * gridDim.x;  // Get the total number of threads in the block.
